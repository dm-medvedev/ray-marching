...
#define MIN_STEP 0.001/10////0.001/100 // 50 хорошая
#define STOP_STEP 0.05/10////0.05/100  // 50 хорошая
#define IMG_ROWS 512*2+1// нечетная
#define IMG_COLS 512*2+1// нечетная
#define CAMERA_IMG_DIST 2
#define PIX_IMG_SIZE 0.1/100// лучше делить на 100 //0.5/3 //расстояние меду двумя соседними ray из камеры на IMG
...
vec3 RayTrace(const Ray& ray, const Scene &scene, std::vector<Light> lights, int count = 0)
{
	...
	//Ambient Occlusion
	color = AmbientOcclusion(hit_point, hit.normal, color, scene, 0.002, 10)*color;//delta, k
...
/*float Fractal::dist(vec3 z)
{
	...
	while (n < this->Iterations) {
		c = a1; dist = length(last_p-a1);
	    d = length(last_p-a2); if (d < dist) { c = a2; dist=d; }
		d = length(last_p-a3); if (d < dist) { c = a3; dist=d; }
		d = length(last_p-a4); if (d < dist) { c = a4; dist=d; }
		last_p = this->Scale*last_p-c*(this->Scale-1.0f);
		n++;
	}

	return length(last_p) * pow(this->Scale, float(-n));
}*/
...
float Fractal::dist(vec3 pos) {
	...
	for (int i = 0; i < this->Iterations ; i++) {
		...
		dr =  pow( r, this->Power-1.0)*this->Power*dr + 1.0;
		
		// scale and rotate the point
		float zr = pow(r, this->Power);
...
int main()
{
	std::vector<Light> lights;
	float intens = 2;
	lights.push_back(Light(vec3(20*intens,40*intens,20*intens), vec3(2.5,-1.5,-0.5)));//vec3(2.5,2.5,2.5/4.0)));
	lights.push_back(Light(vec3(40*intens,20*intens,40*intens), vec3(2.5,2.5,2.5/4.0))); // цвет 10 10 10 хорош
	...
	//Image img(IMG_ROWS, IMG_COLS);
	Matrix <std::tuple<float, float, float>> prev_img(IMG_ROWS, IMG_COLS);
	
	float my_dist = 3.5+2;
	vec3 eye(my_dist+CAMERA_IMG_DIST/3, my_dist/2.0+CAMERA_IMG_DIST/3, my_dist/5.0+CAMERA_IMG_DIST/3); //координаты в мировой системе где камера стоит
	vec3 center(0,0,0);
	vec3 up(0,1,0);
	mat4 M_view = lookAt(eye, center, up); // из мировой к камере
	mat4 inverse_M_view = inverse(M_view);
	...
	save_image(img, "FRACTAL_s_AO.bmp");
	return 0;
}