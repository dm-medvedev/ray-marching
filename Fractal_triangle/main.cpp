...
vec3 RayTrace(const Ray& ray, const Scene &scene, std::vector<Light> lights, int count = 0)
{
	...
	//Ambient Occlusion
	color = AmbientOcclusion(hit_point, hit.normal, color, scene, 0.01, 1)*color;//delta, k
...
Fractal::Fractal(float x, float y, float z, int Iterations, float Scale, 
...
int main()
{
	std::vector<Light> lights;
	float intens = 2;
	lights.push_back(Light(vec3(20*intens,40*intens,20*intens), vec3(0,0,0)));//vec3(2.5,2.5,2.5/4.0)));
