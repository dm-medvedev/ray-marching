...
Fractal::Fractal(float x, float y, float z, int Iterations, float Scale,
	...
	: Object(inverse(glm::translate( glm::mat4(1.0f), vec3(x, y, z))), color, material),
	  x(x), y(y), z(z), Iterations(Iterations), Scale(Scale) {}

float Fractal::dist(vec3 z) {
	vec4 new_p(z.x, z.y, z.z, 1);
	new_p = transform_mat * new_p;
	vec3 last_p(new_p);
	vec3 a1 = vec3(1, 1, 1);
	vec3 a2 = vec3(-1, -1, 1);
	vec3 a3 = vec3(1, -1, -1);
	...
	while (n < Iterations) {
		...
		last_p = Scale * last_p - c * (Scale - 1.0f);
		n++;
	}
	return length(last_p) * pow(Scale, float(-n));
}

int main() {
	std::vector<Light> lights;
	float intens = 2;
	lights.push_back(Light(vec3(20 * intens, 40 * intens, 20 * intens), vec3(0, 0, 0)));
	...
	float my_dist = 3.5 + 2;
	vec3 eye(my_dist + CAMERA_IMG_DIST / 3, my_dist / 2.0 + CAMERA_IMG_DIST / 3, my_dist / 5.0 + CAMERA_IMG_DIST / 3);
	vec3 center(0, 0, 0);
	vec3 up(0, 1, 0);
	mat4 M_view = lookAt(eye, center, up);
	...
	save_image(img, "result/pyramid.bmp");
	return 0;
}